...
def load_dir(dirname):
    # load the content of all .md files in a directory
    # interpreting them as frontmatter files
    # and using pathlib and glob
    posts = []
...
class AllSchoolsWorkflow(Workflow):
    ...
    async def entry(self, ctx: Context, ev: StartEvent) -> SchoolStart:
        # ev.schools is a list of dicts with 'school_name' and 'school_dir'
        # ev.prompts is a dict of strings

        await ctx.set('prompts', ev.prompts)
        await ctx.set('model', ev.model)
        
        await ctx.set('school_count', len(ev.schools))
        await ctx.set('run_dir', ev.run_dir)
        ...
        _, init_usage = await model.astream_chat_to(ev.run_dir / 'init.md', messages, max_tokens=8000, stdout=False, system_prompt=system)
...
async def main(targets_file, run_dir, model):
    # from llama_index.utils.workflow import draw_all_possible_flows
    # draw_all_possible_flows(SeedToGraphWorkflow, filename="tmp/workflow_seedtograph.html")

    # model = AnthropicModel('claude-3.5-sonnet', os.environ.get('ANTHROPIC_API_KEY'))
    ...
    resp = await model.astream_chat_to('test.md', messages, max_tokens=8000, stdout=False)
    print(resp)
    '''

    # read prompts
    prompts = {}
    for prompt in ['0-sys.txt', '1-school.jinja2', '2-shortlist.txt', '3-yaml.txt']:
        prompt_file = Path('prompts') / ('prompt-' + prompt)
        with open(prompt_file) as f:
            prompts[prompt] = f.read()
    
    prompts['cached_system'] = [{
        "type": "text",
        "text": prompts['0-sys.txt'],
    ...
    targets = [
        {
            ...
            "traditions": [
                ...
                {
                    "name": "Roman Philosophy (c. 100 BCE - 300 CE)",
                    "schools": [
...
if __name__ == '__main__':
    run_id = str(datetime.datetime.now(datetime.timezone.utc).strftime("%Y-%m-%d-%H-%M-%S"))
    # run_id = 'v1'
    asyncio.run(main(
        targets_file='prompts/targets-1-full.json',
